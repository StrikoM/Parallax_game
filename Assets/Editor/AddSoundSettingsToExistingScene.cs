...
public class AddSoundSettingsToExistingScene : EditorWindow
{
    ...
    private static void AddSoundSettingsToGameScene(UnityEngine.SceneManagement.Scene scene)
    {
        ...
        if (pausePanel != null)
        {
            // Ищем любые существующие кнопки настроек и переименовываем/чистим
            GameObject settingsBtnObj = null;
            for (int i = 0; i < pausePanel.transform.childCount; i++)
            {
                Transform child = pausePanel.transform.GetChild(i);
                string name = child.name.ToLower();
                if (name.Contains("setting") || name.Contains("настрой"))
                {
                    if (settingsBtnObj == null)
                    {
                        child.name = "SettingsBtn";
                        settingsBtnObj = child.gameObject;
                    }
                    else
                    {
                        // Если есть дубликаты настроек, удаляем их
            ...
            if (settingsBtnObj == null)
            {
                ...
                CreateText("Text", settingsBtnObj.transform, "НАСТРОЙКИ", 30, Color.white, Vector2.zero, new Vector2(500, 80));
            }
            ...
            settingsBtnRt.anchoredPosition = new Vector2(0, 0); // Идеальный центр!

            // Находим ResumeBtn и ExitBtn
            ...
            if (resumeBtnObj != null)
            {
                ...
                // Копируем точные размеры
                RectTransform rRt = resumeBtnObj.GetComponent<RectTransform>();
                if (rRt != null)
                {
                    settingsBtnRt.sizeDelta = rRt.sizeDelta;
                }
            }
            else
            {
                settingsBtnRt.sizeDelta = new Vector2(500, 80);
            }
            ...
            {
                RectTransform rRt = resumeBtnObj.GetComponent<RectTransform>();
                rRt.anchoredPosition = new Vector2(0, 130); // Более просторная верхняя позиция
            }
            ...
            if (exitBtnObj != null && exitBtnObj.transform.parent == pausePanel.transform)
            {
                ...
                eRt.anchoredPosition = new Vector2(0, -130); // Более просторная нижняя позиция
                
                // Копируем размеры с ResumeBtn, чтобы они были абсолютно симметричны
                if (resumeBtnObj != null)
                {
                    RectTransform rRt = resumeBtnObj.GetComponent<RectTransform>();
                    if (rRt != null) eRt.sizeDelta = rRt.sizeDelta;
                }
            }
        }
    ...
    private static void CopyButtonStyle(GameObject sourceBtn, GameObject targetBtn, string newText)
    {
        ...
        if (sourceImage != null && targetImage != null)
        {
            ...
            targetImage.enabled = sourceImage.enabled; // Copy enabled state!
            targetImage.raycastTarget = sourceImage.raycastTarget; // Copy raycastTarget!
        }
        else if (sourceImage == null && targetImage != null)
        {
            targetImage.enabled = false; // Disable image if the source has no Image component
        }
